...
def batched_process(args):
    ...
    in_files_pld = natural_sort(glob.glob(f"{input_dir}*pld*{args.kind}*.gz"))
    ...
    for i in range(len(starts)):
        ...
        max_profile = proc_pyglider_l0(args.glider, args.mission, args.kind, in_sub_dir, out_sub_dir, steps=steps,
...
def process(args):
    ...
    if args.kind == "raw":
        ...
        if adcp_data_present(args.glider, args.mission):
